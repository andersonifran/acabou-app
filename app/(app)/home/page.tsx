            ...
            {currentHouse?.plan === "free" && (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-amber-700 mt-0.5">Locais ilimitados · Pessoas ilimitadas · R$ 6,66/mês</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {actionButtons.map(({ label, sublabel, icon, bg, status }) => (
                  <button
                    ...
                    className={`flex flex-col items-start p-4 rounded-2xl border-2 transition-all hover:scale-[1.02] active:scale-[0.98] ${bg}`}
                  >
                    <div className="mb-3">{icon}</div>
                    <span className="font-bold text-gray-900 text-sm leading-tight">{label}</span>
                    <span className="text-xs text-gray-500 mt-0.5 leading-tight">{sublabel}</span>
                  </button>
                ))}
              </div>
            </div>
